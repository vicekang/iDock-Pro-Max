...
4. 在 Codex 中指示查找联系人、拨打指定号码、发送指定短信或通过模块上网。
...
最近 200 个操作去重记录保存参数哈希和结果；最近 300 条事件及通话文字保留在内存，重启清空。原应用的短信、联系人和电话记录沿用原存储。不要上传运行目录、SIM 标识、号码、短信、联系人、令牌或签名私钥。
